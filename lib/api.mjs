...
export const mfssia = {
  ...
  // Per-patient allergy record (built from the DKG graph) — { substances, substanceIds, ... }.
  patientRecord: (patientId) => mreq('GET', `${cfg.mfssiaApi}/patient-record/${patientId}/proof`),
...
// ── EVM proof verification (isolated on-chain Groth16 verify — the contribution slice) ──
export const evm = {
  verify: (proof, publicSignals) => req('POST', `${cfg.evmApi}/verify`, { proof, publicSignals }),
};
...
export async function ensureConsentAnchored(
  ...
  const t0 = Date.now();
  console.log(`[consent] granted ${organizationId}; waiting for it to anchor in the DKG (up to ${Math.round(timeoutMs / 1000)}s)...`);
  while (Date.now() < deadline) {
    const r = await req('POST', `${cfg.mfssiaApi}/physician-access/check`, { doctorId, patientId });
    if (r.body?.data?.authz === true || r.body?.authz === true) {
      console.log(`[consent] anchored after ${Math.round((Date.now() - t0) / 1000)}s`);
      return true;
    }
    console.log(`[consent] not yet anchored (t+${Math.round((Date.now() - t0) / 1000)}s)...`);
    await new Promise((res) => setTimeout(res, 5000));
  }
  console.log(`[consent] gave up after ${Math.round(timeoutMs / 1000)}s`);
  return false;
}
